...
const connectDB = async () => {
  try {
    await mongoose.connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/familysite', {
      useNewUrlParser: true,
      useUnifiedTopology: true
    });
    console.log('MongoDB 연결 성공');
  } catch (error) {
    console.log('MongoDB 연결 실패, 메모리 모드로 실행:', error.message);
    // MongoDB 없이도 서버가 계속 실행되도록 함
  }
};

connectDB();

// 임시 메모리 저장소 (MongoDB 없을 때 사용)
let memoryUsers = [];
let memoryMessages = [];
let userIdCounter = 1;
...
app.post('/api/register', async (req, res) => {
  try {
    ...
    // MongoDB가 연결되어 있으면 DB 사용, 아니면 메모리 사용
...
app.post('/api/login', async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      { userId: user._id || user.id, username: user.username, familyCode: user.familyCode },
...
app.get('/api/messages/:room', authenticateToken, async (req, res) => {
  try {
    ...
    // MongoDB가 연결되어 있으면 DB 사용, 아니면 메모리 사용
    if (mongoose.connection.readyState === 1) {
      messages = await Message.find({ room: req.params.room })
        .sort({ timestamp: -1 })
        .limit(50);
    } else {
      // 메모리 저장소 사용
